...
class ValueNorm(nn.Module):
    ...
    def update(self, input_vector):
        ...
        if len(input_vector.shape)==3:
            input_vector = torch.reshape(input_vector, (input_vector.shape[0]*input_vector.shape[1],input_vector.shape[2]))
    ...
    def normalize(self, input_vector):
        ...
        if type(input_vector) == np.ndarray:
            input_vector = torch.from_numpy(input_vector)
        origin_shape = input_vector.shape
        if len(origin_shape)==3:
            input_vector = torch.reshape(input_vector, (origin_shape[0]*origin_shape[1],origin_shape[2]))
        input_vector = input_vector.to(self.running_mean.device)  # not elegant, but works in most cases

        mean, var = self.running_mean_var()
        out = (input_vector - mean[(None,) * self.norm_axes]) / torch.sqrt(var)[(None,) * self.norm_axes]
        if len(origin_shape)==3:
            out = torch.reshape(out, origin_shape)
        return out
